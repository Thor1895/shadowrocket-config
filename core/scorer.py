...
from core.constants import NODE_SCORE_FILE, OPENAI_HEALTH_JSON, REGION_PENALTIES, ROOT
...
def load_health(path: Path = OPENAI_HEALTH_JSON) -> dict[str, dict[str, Any]]:
    if not path.exists():
        return {}
...
def write_scores(
    output_path: Path = NODE_SCORE_FILE,
    health_path: Path = OPENAI_HEALTH_JSON,
    limit: int = 3,
) -> Path:
    nodes = load_nodes()
...
def load_top_nodes(path: Path = NODE_SCORE_FILE, limit: int = 3) -> list[str]:
    if not path.exists():
        return []
    payload = json.loads(path.read_text(encoding="utf-8"))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", type=Path, default=NODE_SCORE_FILE, help="Node score JSON output path.")
    parser.add_argument("--health", type=Path, default=OPENAI_HEALTH_JSON, help="OpenAI health JSON input path.")
